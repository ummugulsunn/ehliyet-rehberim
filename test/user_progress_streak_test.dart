...
void main() {
  ...
  group('UserProgressRepository Streak Logic', () {
    ...
    test(
      'Case 1: Consecutive Day (Yesterday -> Today) should INCREMENT streak',
      () async {
        ...
        expect(
          ...
          6,
          reason: "Streak should increment from 5 to 6",
        );
        expect(
          prefs.getString('last_completed_date'),
          formatDate(DateTime.now()),
          reason: "Date should update to today",
        );
      },
    );
    ...
    test('Case 2: Same Day (Today -> Today) should MAINTAIN streak', () async {
      ...
      expect(
        prefs.getInt('current_streak'),
        5,
        reason: "Streak should remain 5",
      );
    });

    test(
      'Case 3: Missed Day (2 Days Ago -> Today) WITHOUT Freeze should RESET streak',
      () async {
        ...
        expect(
          prefs.getInt('current_streak'),
          1,
          reason: "Streak should reset to 1 (current day active)",
        );
      },
    );

    test(
      'Case 4: Missed Day (2 Days Ago -> Today) WITH Freeze should PRESERVE streak',
      () async {
        final twoDaysAgo = DateTime.now().subtract(const Duration(days: 2));
        ...
        expect(
          prefs.getInt('current_streak'),
          11,
          reason: "Streak should increment (preserved)",
        );
        expect(
          prefs.getInt('streak_freezes'),
          0,
          reason: "Freeze should be consumed",
        );
      },
    );

    test(
      'Case 5: Missed 2 Days (3 Days Ago -> Today) WITH 1 Freeze should RESET streak',
      () async {
        ...
        expect(
          prefs.getInt('current_streak'),
          1,
          reason: "Streak should reset because not enough freezes",
        );
        expect(
          prefs.getInt('streak_freezes'),
          1,
          reason: "Freeze should NOT be consumed if streak breaks",
        );
      },
    );
  });
