...
final filteredTrafficSignsProvider = FutureProvider<List<TrafficSignCategory>>((ref) async {
  // Get the original data
  final allCategories = await ref.watch(trafficSignsProvider.future);
  
  // Get current filter states
  final searchQuery = ref.watch(trafficSignSearchQueryProvider).toLowerCase().trim();
  ...
  if (searchQuery.isNotEmpty) {
    filteredCategories = filteredCategories.map((category) {
      ...
      return filteredSigns.isNotEmpty
          ...
          : null;
    }).where((category) => category != null).cast<TrafficSignCategory>().toList();
  }
