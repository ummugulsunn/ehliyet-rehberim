...
class FavoritesRepository {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _isInitialized = true;
      Logger.info('FavoritesRepository initialized with ${_cachedFavorites.length} items');
    } catch (e) {
  ...
  String? getNote(int questionId) {
    try {
      final item = _cachedFavorites.firstWhere((item) => item.questionId == questionId);
      return item.note;
    } catch (_) {
      return null;
    }
  ...
  Future<void> toggleFavorite(int questionId) async {
    final existingIndex = _cachedFavorites.indexWhere((item) => item.questionId == questionId);
  ...
  Future<void> saveNote(int questionId, String note) async {
    ...
    if (existingIndex >= 0) {
      ...
      _cachedFavorites[existingIndex] = _cachedFavorites[existingIndex].copyWith(
        note: note,
  ...
  Future<void> _persistChanges() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final jsonString = jsonEncode(_cachedFavorites.map((e) => e.toJson()).toList());
